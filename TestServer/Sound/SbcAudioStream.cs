...
namespace TestServer.Sound
{
    public class SbcAudioStream: IDisposable
    {
        ...
        public SbcAudioStream()
        {
            ...
            _resampleRatio = SampleRate / (double) _captureDevice.WaveFormat.SampleRate;
        ...
        private unsafe void CaptureDeviceOnDataAvailable(object sender, WaveInEventArgs e)
        {
            ...
            {   
                ...
                {
                    if (SampleRate != _captureDevice.WaveFormat.SampleRate)
                    {
                ...
                var prevLen = _sbcAudioData.CurrentLength;
                ...
                // lock (_sbcAudioData)
                //     Console.WriteLine("framesAvailable: {0}", _sbcAudioData.CurrentLength / (int) _encoder.FrameSize);

                var now = _watch.ElapsedTicks;
                var diff = now - _lastLap;
                _lastLap = now;
                var elapsedTime = Math.Round((diff / (double) Stopwatch.Frequency) * 100000000.0) / 100.0;
                
                // Console.WriteLine("frames: {0}, frameSize: {5}, prevLen: {4}, length: {1}, consumed: {2}, elapsed: {3}", framesAvailable, _sbcAudioData.CurrentLength, sbcFrameDataLength, elapsedTime, prevLen, (int) _encoder.FrameSize);
            }
        ...
        // public void CopyEncodedFramesTo(byte[] destination, int frameCount, int offset)
        // {
        //     var dataSize = frameCount * (int) _encoder.FrameSize;
        //     
        //     WaitUntil(dataSize);
        //
        //     lock (_sbcAudioData)
        //     {
        //         _sbcAudioData.CopyTo(destination, offset, dataSize);
        //     }
        // }

...
namespace bt2usb.Linux.Udev
{
    ...
    {
        private readonly Lazy<Device> lazyParent;
        ...
        public string Driver
        {
            get
            {
                ...
                if (driver == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string DevType
        {
            get
            {
                ...
                if (devtype == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string Subsystem
        {
            get
            {
                ...
                if (subsystem == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string this[string key]
        {
            get
            {
                ...
                {
                    var err = Marshal.GetLastWin32Error();
                    if (err > 0) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        internal Context Context
        {
            get
            {
                ...
                if (udev == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string DevPath
        {
            get
            {
                ...
                if (devpath == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string SysPath
        {
            get
            {
                ...
                if (syspath == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string SysName
        {
            get
            {
                ...
                if (sysname == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string SysNum
        {
            get
            {
                ...
                if (sysnum == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());
        ...
        public string DevNode
        {
            get
            {
                ...
                if (devnode == IntPtr.Zero)
                {
                    ...
                    if (err != Errno.ENOENT) throw new UnixIOException(err);
        ...
        public IEnumerable<KeyValuePair<string, string>> Properties
        {
            get
            {
                ...
                {
                    var err = (Errno) Marshal.GetLastWin32Error();
                    if (err != Errno.ENOENT) yield break;
        ...
        public string Action
        {
            get
            {
                ...
                {
                    var err = (Errno) Marshal.GetLastWin32Error();
                    if (err == Errno.ENOENT) return null;
        ...
        /// </remarks>
        public bool IsInitialized => udev_device_get_is_initialized(Handle);
        ...
        private static extern ulong udev_device_get_seqnum(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_driver(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_devtype(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_subsystem(IntPtr udev_device);
        ...
        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_parent(IntPtr udev_device);

        private Device getParent()
        {
            var parent = udev_device_get_parent(Handle);
            if (parent == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());

            return new Device(udev_device_ref(parent));
        }

        [DllImport(UdevLibraryName, CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern IntPtr udev_device_get_parent_with_subsystem_devtype(IntPtr udev_device, string subsystem,
            string devtype);
        ...
        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_udev(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_devpath(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_syspath(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_sysname(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_sysnum(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_devnode(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_devlinks_list_entry(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_properties_list_entry(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_action(IntPtr udev_device);

        [DllImport(UdevLibraryName, CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern IntPtr udev_device_get_sysattr_value(IntPtr udev_device,
        ...
        private static extern int udev_device_set_sysattr_value(IntPtr udev_device, string sysattr, string value);
        ...
        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_sysattr_list_entry(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern bool udev_device_get_is_initialized(IntPtr udev_device);

        [DllImport(UdevLibraryName, SetLastError = true)]
        private static extern IntPtr udev_device_get_tags_list_entry(IntPtr udev_device);

        [DllImport(UdevLibraryName, CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern bool udev_device_has_tag(IntPtr udev_device, string tag);
        ...
        [DllImport(UdevLibraryName, CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern IntPtr udev_device_new_from_syspath(IntPtr udev_device, string syspath);

        private static IntPtr New(Context udev, string syspath)
        {
            if (udev == null) throw new ArgumentNullException(nameof(udev));

            if (syspath == null) throw new ArgumentNullException(nameof(syspath));

            var device = udev_device_new_from_syspath(udev.Handle, syspath);
            if (device == IntPtr.Zero) throw new UnixIOException(Marshal.GetLastWin32Error());

            return device;
        }

        [DllImport(UdevLibraryName, CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern IntPtr udev_device_new_from_subsystem_sysname(IntPtr udev_device, string subsystem,
            string sysname);

        private static IntPtr New(Context udev, string subsystem, string sysname)
        {
            if (udev == null) throw new ArgumentNullException(nameof(udev));

            if (subsystem == null) throw new ArgumentNullException(nameof(subsystem));

            if (sysname == null) throw new ArgumentNullException(nameof(sysname));
        ...
        private static extern IntPtr udev_device_ref(IntPtr udev_device);
        ...
        [DllImport(UdevLibraryName)]
        private static extern IntPtr udev_device_unref(IntPtr udev_device);
